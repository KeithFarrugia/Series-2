...
In the real world, there are many limitOrder types, but for this assignment, the limitOrder types you will be implementing are limit orders. An limitOrder contains information about which stock to buy, how many shares, and - in the case of limit orders - at what price. The role of the price in a limit limitOrder depends on whether it is a buy or a sell limitOrder:

 - For buy orders, the price in a limit limitOrder denotes the maximum share price the trader wishes to buy their shares at.
 - For sell orders, the price in a limit limitOrder denotes the minimum share price a trader wishes to sell their shares for.

When a new limitOrder comes in, it will first try to match it against existing orders. For sell orders, this means finding the buy limitOrder with the highest price (provided that this price is higher than the limit price). For buy orders, this means finding the sell limitOrder with the lowest price (provided that this price is lower than the limit price). The exchange will keep trying to do this until the limitOrder is resolved. When no matching orders can be found (anymore), the buy/sell limitOrder should be added to the bids/asks.
Each time an limitOrder is (partially) resolved, the transaction is added to the transaction history of the trader who placed the limitOrder. A transaction contains information on how many of which stock was traded for how much.

The price of a stock is updated whenever a sell limitOrder for that stock (partially) resolves. The price is then updated to the price that was paid at the latest transaction. As an example, suppose we have a sell limitOrder in our asks for 10 shares at \$208. This means the seller is prepared to sell the stock for a minimum of \$208. When a new buy limitOrder comes with a limit price of \$204, the limitOrder cannot be matched, since the buyer is only prepared to pay at most \$204. As such, this buy limitOrder is added to the bids. When another buy limitOrder comes in for 10 shares with a price limit of \$209, it will be matched with the existing sell limitOrder for the price of the sell limitOrder. This means the limitOrder will be resolved for \$208 dollars and the price of the stock will be updated to \$208 (since this is the price of the latest transaction). Similarly, if a sell limitOrder for \$202 would come in, it would be matched against the buy limitOrder of \$204, and the price of the stock is updated to \$204.

While you only need to worry about limit orders, you should design your program in such a way that adding support for more limitOrder types is relatively easy.
...
Similar to Gson, in limitOrder to get Jackson to work with our module structure, we need to `open` the package of the class we are trying to load to the Jackson library. Suppose the class we are trying to load is located in a package called `stocks` in an `example` module, then we would have to add the following at the end of the `module-info.java` of the `example` module:
...
- Each trader bot should send a stock limitOrder to the message queue of the `Stock Application` every `x` seconds. How often exactly can be randomized; e.g. something between 1 and 4 seconds.
- The stock limitOrder that a trader bot made is determined by a trading strategy (more on this later).
...
Determining what kind of limitOrder can be made can be quite complicated. Luckily, you don't have to write a complicated algorithm for this; a simple random strategy is perfectly fine. An example strategy could be:
...
Pay attention that the trader cannot sell stocks they do not own. When a trader makes a sell limitOrder, the shares will stay in their portfolio until that limitOrder is resolved and the trader is informed. As such, they should not be able to sell those same shares again while the limitOrder has not been resolved yet (as this would mean you could sell the same share twice). A trader bot should also not be able to buy stocks it does not have the money for.
